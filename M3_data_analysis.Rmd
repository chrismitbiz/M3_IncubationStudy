...
H0 <- dataCHN %>% filter(Week == "T0") %>% select(HC)
...
dfCHN <- data.frame(Treatment = data_part1[c(1:44),]$Treatment,
                 H52  = H52$H,
                 HC52  = HC52$HC,
                 CN52  = CN52$CN,
                 C52  = C52$C,
...
p1 <- ggcorrplot::ggcorrplot(cor(dfCHN %>% dplyr::select(-Treatment,-RespCumT48, -RespCumT46, -RespCumT44)), type = 'lower')
...
## Scatter plot of respiration to dieldrin loss (%) and CN ratio
...
## CHN dataframe from chunk 'corplotfig5', creating a scatterplot of CN vs respiration to sit next to the scatterplot
p3 <- dfCHN %>% ggpubr::ggscatter(y = "TotalRespiration", x = "CN52",color = "Treatment", palette = "simpsons", ylab = "Total microbial respiration (µg CO2-C / g soil)", xlab = "Carbon-to-Nitrogen ratio after 52 weeks") + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

p4 <- dfCHN %>% ggpubr::ggscatter(y = "TotalRespiration", x = "CN39",color = "Treatment", palette = "simpsons", ylab = "Total microbial respiration (µg CO2-C / g soil)", xlab = "Carbon-to-Nitrogen ratio after 39 weeks") + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))
