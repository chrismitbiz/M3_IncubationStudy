...
```{r dataCHN, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=FALSE}
...
dataCHN <- dataCHN %>% rstatix::convert_as_factor(SampleID, Treatment, Week)
#order factors
...
T0 <- dataCHN %>% filter(Week == "T0")
T16 <- dataCHN %>% filter(Week == "T16")
T39 <- dataCHN %>% filter(Week == "T39")
T52 <- dataCHN %>% filter(Week == "T52")
dataCHN <- dataCHN %>% 
  mutate(Closs = (T0$C - C)) 
...
```{r boxplotsWATER, fig.width=10, fig.asp=0.8, message=FALSE, echo = FALSE, warning = FALSE, cache=TRUE, fig.cap="Boxplots of watercontent."}
