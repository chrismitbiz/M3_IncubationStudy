...
```{r setcols, echo=FALSE}
#cols <- c("#FED439FF", "#FD8CC1FF", "#C80813FF", "#D5E4A2FF","#D2AF8199",  "#D2AF81FF","#71D0F5FF",  "#709AE1FF", "#00008B" , "#91331FFF", "#383E42")

#manually created with help of https://htmlcolorcodes.com/
cols <- c("#F9E79F", "#FDEBD0", "#F6DDCC", "#82E0AA", "#E5E8E8", "#CCD1D1", "#AAB7B8", "#707B7C", "#566573", "#641E16", "#8E44AD")

#cols_ref <- c("#FED439FF", "#FD8CC1FF", "#C80813FF", "#D2AF8199",  "#D2AF81FF","#71D0F5FF", "#D5E4A2FF", "#709AE1FF", "#00008B" , "#91331FFF", "#383E42", "#FED439FF")
```
...
```{r dieldrindataframe, message=FALSE, echo = FALSE, warning = FALSE, cache=FALSE, include=TRUE}
library(hrbrthemes)
library(ggpubr)
library(tidyverse)
library(rstatix)
...
data$Treatment <- factor(data$Treatment, levels=c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate","CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar","Reference"))
...
# Include the soil sampled (g) and soil amunt left after sampling (g) 
soilweights <- read.csv("~/Documents/Study/LaTrobe/Research/phD/Milestone3/M3_IncubationStudy/data.csv") %>% select(SoilSampled_g, Soilremaining_g)
data_noref$SoilSampled_g <- soilweights$SoilSampled_g 
data_noref$Soilremaining_g <- soilweights$Soilremaining_g
data_noref$Soilremaining_dry <- data_noref$Soilremaining_g / (1 + data_noref$H20) 

data_fltrd  <- data_noref %>% 
 # filter(SampleName != "t0_4.run") %>% 
  filter(Treatment != "Reference")

# Summary stats for some variable 
summary <- data_noref %>% dplyr::select(Week, Treatment, H20, Soilremaining_g, Soilremaining_dry) %>% group_by(Week, Treatment)  %>% rstatix::get_summary_stats(type = "mean_se")
...
dataCO2$Treatment <- factor(dataCO2$Treatment, levels=c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate","CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar"))
...
data_part2 <- dataCO2 %>% rownames_to_column("id") %>% dplyr::filter(id %in% 353:nrow(dataCO2)) %>% dplyr::mutate(µgCO2gsoilcum = CO2cum$value)
...
dataCO2 <- dataCO2 %>%
  ...
  dplyr::mutate(cum_µgCO2.Cgsoil = cumsum(µgCO2.Cgsoil))
...
dataCHN$Treatment <- factor(dataCHN$Treatment, levels=c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate","CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar"))
...
# Get summary stats
#summary <- dataCHN %>% group_by(Week, Treatment) %>%  rstatix::get_summary_stats(type = "mean_se")
```

```{r dataMBC, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=TRUE}
library(tidyverse)
dataMBC <- read.csv("~/Documents/Study/LaTrobe/Research/phD/Milestone3/Data_MBC/Data_MBC.csv", header = TRUE, row.names=NULL)
dataMBC <- dataMBC %>% dplyr::filter(WAT %in% c("Reference", "T0","T39","T52"))
dataMBC$Treatment[dataMBC$Treatment == "AutoclavedControl"] <- "Heat"
dataMBC  <- dataMBC %>% 
  dplyr::select(id, SampleNo, Treatment, TOC.g.kg, WAT, mgC_gdrySoil, MBC_gkg_drysoil, MBC_µgg_drysoil, MBC_pcntTOC) 

dataMBC$MBC_µgg_drysoil  <-  ifelse(dataMBC$MBC_µgg_drysoil == 0, 1, dataMBC$MBC_µgg_drysoil)

dataMBC <- dataMBC %>% 
  dplyr::filter(mgC_gdrySoil > 0.00) %>%
  dplyr::mutate_all(type.convert) %>% 
  dplyr::mutate_if(is.factor, as.character) %>% 
  rename(Week = "WAT")  %>% 
  dplyr::filter(mgC_gdrySoil > 0.00) 

dataMBC <- dataMBC %>% mutate(Week_int = case_when(Week == "T39" ~ 39,
                              Week == "T52" ~ 52,
                              TRUE ~0))

dataMBC$MBC_Cratio <-  dataMBC$MBC_gkg_drysoil / dataMBC$TOC.g.kg
#dataMBC <- dataMBC %>% rstatix::convert_as_factor(SampleNo, Treatment)
#order factors
#dataMBC$Treatment <- factor(dataMBC$Treatment, levels=c("Control", "Lime", "PoultryLitter", "Biochar", "CitricAcid", "Inositol", "SodiumFormate", "SodiumAcetate","FumaricAcid", "Heat","H2O2", "Reference"))

dataMBC$Treatment <- factor(dataMBC$Treatment, levels=c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate","CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar"))

dataMBC$Week <- factor(dataMBC$Week, levels=c("T0","T39","T52","Reference"))

```

```{r dataunited, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=TRUE }
data_united <- dplyr::inner_join(data_noref, dataCHN, by = "id") %>% select(id, Week.x, Week_int.x, Treatment.x, 
                                                                            Category, Dieldrin, Dieldrin_trans, loss, loss39, H20, H20_losssinceT0, C, H, N, CN, HC, Closs, Soilremaining_dry) 

## EXLUCDING T16 ###
data_united <- dplyr::inner_join(data_united, dataMBC, dataCHN, by = "id") %>% dplyr::select(-Treatment.x, - Week.x, -Week_int.x, -Category) %>% drop_na( )
```

```{r dataframeallcombined, echo = FALSE, warning = FALSE, cache=TRUE, include=TRUE, fig.width=8, fig.asp=1 }
...
df2 <-  df2 %>% 
  dplyr::rename(H20lossT0 = T0) %>% 
  dplyr::rename(H20lossT16 = T16) %>% 
  dplyr::rename(H20lossT39 = T39) %>% 
  dplyr::rename(H20lossT52 = T52) 

#str(data_noref)

H52 <- dataCHN %>% filter(Week == "T52") %>% select(H)
HC52 <- dataCHN %>% filter(Week == "T52") %>% select(HC)
CN52 <- dataCHN %>% filter(Week == "T52") %>% select(CN)
C52 <- dataCHN %>% filter(Week == "T52") %>% select(C)

H39 <- dataCHN %>% filter(Week == "T39") %>% select(H)
HC39 <- dataCHN %>% filter(Week == "T39") %>% select(HC)
CN39 <- dataCHN %>% filter(Week == "T39") %>% select(CN)
C39 <- dataCHN %>% filter(Week == "T39") %>% select(C)

H0 <- dataCHN %>% filter(Week == "T0") %>% select(H)
HC0 <- dataCHN %>% filter(Week == "T0") %>% select(HC)
CN0 <- dataCHN %>% filter(Week == "T0") %>% select(CN)
C0 <- dataCHN %>% filter(Week == "T0") %>% select(C)

MBC_pcntTOC0 <- (dataMBC %>% filter(Week == "T0"))$MBC_pcntTOC
MBC_pcntTOC39 <- (dataMBC %>% filter(Week == "T39"))$MBC_pcntTOC
MBC_pcntTOC52 <- (dataMBC %>% filter(Week == "T52"))$MBC_pcntTOC

MBC_µgg_drysoil0 <- (dataMBC %>% filter(Week == "T0"))$MBC_µgg_drysoil
MBC_µgg_drysoil39 <- (dataMBC %>% filter(Week == "T39"))$MBC_µgg_drysoil
MBC_µgg_drysoil52 <- (dataMBC %>% filter(Week == "T52"))$MBC_µgg_drysoil

MBC_RespRatio0 <-  df1$RespCumT52 / (dataMBC %>% filter(Week == "T0"))$MBC_µgg_drysoil 
MBC_RespRatio39 <-  df1$RespCumT52 / (dataMBC %>% filter(Week == "T39"))$MBC_µgg_drysoil 
MBC_RespRatio52 <-  df1$RespCumT52  / (dataMBC %>% filter(Week == "T52"))$MBC_µgg_drysoil 

EOCT0 <- (dataMBC %>% filter(Week == "T0"))$mgC_gdrySoil
EOC39 <- (dataMBC %>% filter(Week == "T39"))$mgC_gdrySoil
EOC52 <- (dataMBC %>% filter(Week == "T52"))$mgC_gdrySoil

H20T0 <- (data_united %>% filter(Week == "T0"))$H20
H20T39 <- (data_united %>% filter(Week == "T39"))$H20
H20T52 <- (data_united %>% filter(Week == "T52"))$H20


#Create a df to compare the sum of CO2 produced with other variables
df_combined <- data.frame(Treatment = data_noref[c(1:44),]$Treatment,
                 MBC_pcntTOC0 = MBC_pcntTOC0,
                 MBC_pcntTOC39 = MBC_pcntTOC39,
                 MBC_pcntTOC52 = MBC_pcntTOC52,
                 MBC_RespRatio0 = MBC_RespRatio0,
                 MBC_RespRatio39 =  MBC_RespRatio39,
                 MBC_RespRatio52 =  MBC_RespRatio52,
                 EOCT0 = EOCT0,
                 EOC39 = EOC39,
                 EOC52 = EOC52,
                 H0    = H0$H,
               ...
               #PeakareaT39 = as_vector(data_noref %>% dplyr::filter(Week == "T39") %>% dplyr::select(GCArea_Dieldrin)),
                 DieldrinT0 = as_vector(data_noref %>% dplyr::filter(Week == "T0") %>% dplyr::select(Dieldrin)),
...
#test <- df_combined %>% rstatix::get_summary_stats()

## Difference of Soil variables from ## T0 to T52 ##
Closs <- ((C0 - C52) / C0) * 100
EOCloss <-  ((EOCT0 - EOC52) / EOCT0) * 100
CN_change <- ((CN0 - CN52)/ CN0) * 100
Hloss <- ((H0 - H52) / H0) * 100
HC_change <- ((HC0 - HC52)/ HC0) * 100 

# change negatives into 0 to assure the substraction is done correctly.
MBC_µgg_drysoil52 <- ifelse(MBC_µgg_drysoil52 < 0, 0, MBC_µgg_drysoil52)
MBC_decrease <- ((MBC_µgg_drysoil0 - MBC_µgg_drysoil52)  / MBC_µgg_drysoil0) * 100 

H20_change <- ((H20T0 - H20T52) / H20T0) * 100


df_change0_52  <- data.frame(Treatment = data_noref[c(1:44),]$Treatment,
                 Closs = Closs$C,
                 EOCloss = EOCloss,
                 CN_change = CN_change$CN,
                 Hloss = Hloss$H,
                 HC_change = HC_change$HC,
                 MBC_decrease = MBC_decrease,
                 H20_change = H20_change,
                 Resp_cum08 = (dataCO2 %>% dplyr::filter(Week == "T08") %>%  ungroup() %>% 
                                 dplyr::select(cum_µgCO2.Cgsoil))$cum_µgCO2.Cgsoil,
                 Resp_cum16 = (dataCO2 %>% dplyr::filter(Week == "T16") %>%  ungroup() %>% 
                                 dplyr::select(cum_µgCO2.Cgsoil))$cum_µgCO2.Cgsoil,
                 Resp_cum52 = (dataCO2 %>% dplyr::filter(Week == "T52") %>%  ungroup() %>% 
                                 dplyr::select(cum_µgCO2.Cgsoil))$cum_µgCO2.Cgsoil,
                 TotalLossT39 = as_vector(data_noref %>% dplyr::filter(Week == "T39") %>% dplyr::select(loss)),
                 TotalLossT52 = as_vector(data_noref %>% dplyr::filter(Week == "T52") %>% dplyr::select(loss)),
                 MBC_RespRatio0 = MBC_RespRatio0,
...
#car::scatterplotMatrix(df_change0_52)
```

# Amendments 
**Aimed at modifying carbon metabolism**
```{r treatmentCfig1, fig.cap="Treatments ordered by amount of carbon added to soil", echo = FALSE, warning = FALSE, cache=FALSE, include=TRUE}
treatmentC <- read.csv("~/Documents/Study/LaTrobe/Research/phD/Milestone3/treatmentC.csv", header = TRUE, row.names=NULL)
treatmentC$Treatment[treatmentC$Treatment == "AutoclavedControl"] <- "Heat"

treatmentC$Treatment <- factor(treatmentC$Treatment, levels=c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate", "CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar"))

treatmentC$SubCtoTotC <- treatmentC$TotalSubstrateC / treatmentC$TotalCpostT_g * 100
treatmentC <-  treatmentC %>% dplyr::arrange(match(Treatment, c("Control", "H2O2","Heat", "Lime","Inositol", "SodiumFormate", "SodiumAcetate", "CitricAcid", "FumaricAcid",  "PoultryLitter", "Biochar")))

treatmentC  %>% 
  ggpubr::ggbarplot(data=., x = "Treatment", y = "SubCtoTotC", fill = "Treatment", palette = cols,  xlab = FALSE, ylab = "Substrate-C (% TOC)") +
  rotate_x_text(angle = 45) +
  ggtitle("Substrate-C added at start (T0)")
...
```{r dieldrinsummary, fig.width=10, fig.asp=0.4, message=FALSE, echo = FALSE, warning = FALSE, cache=FALSE, include=FALSE}
...
```{r boxplotsfig2, fig.width=10, fig.asp=0.8, message=FALSE, echo = FALSE, warning = FALSE, cache=TRUE, fig.cap="Boxplots of dieldrin concentrations and dieldrin concentrations relative to start of experiment (T0), including histograms."}
...
p3 <- ggboxplot( data_fltrd %>% 
                   filter(SampleName != "#2.run" ) %>%  # removing T16_02 (Control) sample, this was measured without internal standard and varied significantly 
                   filter(SampleName != "averageofthree" ), # removing T16_31 (Sodium formate), this was measured without internal standard and appeared to have failed 
                 x = "Week", y = "Dieldrin_trans", ylab = "Dieldrin (log)", fill = "Treatment", palette = cols)+ rotate_x_text(angle = 45)
...
l0 <- data_noref %>% 
  #filter(SampleName != "t0_4.run" ) %>%
   filter(SampleName != "#2.run" ) %>%  # removing T16_02 (Control) sample, this was measured without internal standard and varied significantly 
  filter(SampleName != "averageofthree" ) %>% # removing T16_31 (Sodium formate), this was measured without internal standard and appeared to have failed
  ggpubr::ggline(data = ., x = "Week_int", y = "Dieldrin_trans", ylab = "Dieldrin (log)", add = c("mean_se"), color = "Treatment", palette = cols) + 
...
s0 <- data_noref %>% 
  ...
  ggpubr::ggscatter(data = ., x = "Week_int", y = "Dieldrin_trans", ylab = "Dieldrin (log)", add = "reg.line", color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")
...
## Get the slopes of linear models
  
#models <- data_noref %>% 
#nest(data = -Treatment) %>%   
#mutate(
#    fit = map(data, ~ lm(Dieldrin_trans ~ Week_int, data = .x)),
#    tidied = map(fit, tidy)
#  ) %>% 
#  unnest(tidied)

#models %>% filter(term == "Week_int") %>% dplyr::select(estimate) %>% rstatix::get_summary_stats()
...
# Soil variables change over time
...
CNplot <- dataCHN %>% 
  ...
  ggpubr::ggboxplot(data =., x = "Treatment", y = "CN",  fill = "Treatment", palette = cols, legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
...
HCplot <- dataCHN %>% 
  #filter(Week %in% c("T0","T52")) %>% 
  filter(Week == "T52") %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "HC",  fill = "Treatment", palette = cols, 
...
Cplot <- dataCHN %>% 
  #filter(Week %in% c("T0","T52")) %>%  
  filter(Week == "T52") %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "C",  fill = "Treatment", palette = cols, legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
...
Nplot <- dataCHN %>% 
  filter(Week == "T52") %>% 
  filter(Week %in% c("T52")) %>%
  ggpubr::ggboxplot(data =., x = "Treatment", y = "N",  fill = "Treatment", palette = cols, legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
...
Hplot <- dataCHN %>% 
  filter(Week == "T52") %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "H",  fill = "Treatment", palette = cols,  legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
...
Clossplot <- dataCHN %>% 
  filter(Week == "T52") %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "Closs",  fill = "Treatment", palette = cols,  legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
...
Hplot <- dataCHN %>% 
  filter(Week == "T0") %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "H",  fill = "Treatment", palette = cols, facet.by = "Week", legend = "none",add = c("jitter"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
  ggpubr::stat_compare_means(method = "kruskal", label.y = 1.3) +
  annotate("text", x = groups1$Treatment, y=c(1.28,1.28,1.28,1.28,1.28,1.28,1.28,1.28,1.28,1.28,1.28), label = label) + 
...
```{r lineplotfig4, fig.cap="Soil variables change over time", message=FALSE, echo = FALSE, warning = FALSE, include=TRUE, eval=TRUE, fig.width= 12, fig.asp=0.7}
...
p1 <- df %>% 
  ggpubr::ggline(data =., x = "Week", y = "CN",  color = "Treatment", palette = cols,add = c("mean_se"), ylab = "C:N ratio") + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p2<- df %>%  
  ggpubr::ggline(data =., x = "Week", y = "C",  color = "Treatment", palette = cols, add = c("mean_se"), ylab = "Total-C (%)") + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

#p3 <- df %>% 
#  ggpubr::ggline(data =., x = "Week", y = "N",  color = "Treatment", palette = cols , add = c("mean_se"))

p4 <- df %>% 
  ggpubr::ggline(data =., x = "Week", y = "HC",  color = "Treatment", palette = cols, add = c("mean_se"),ylab = "Hydrogen:TotalC ratio") + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

#p5 <-df %>% 
#  ggpubr::ggline(data =., x = "Week", y = "H",  color = "Treatment", palette = cols, add = c("mean_se")) 

p6 <-df %>% 
  ggpubr::ggline(data =., x = "Week", y = "Dieldrin_trans",  color = "Treatment", palette = cols, add = c("mean_se"), ylab = "log(Dieldrin concentration µg/g)")  + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p7 <-df %>% 
  ggpubr::ggline(data =., x = "Week_int", y = "MBC_pcntTOC",  color = "Treatment", palette = cols, add = c("mean_se"),ylab = "Micobialbiomass-C (% of TOC)")  + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p8 <- dataCO2 %>% 
  ggpubr::ggline(data =., x = "Week_int", y = "cum_µgCO2.Cgsoil", color = "Treatment",   palette = cols, add = c("mean_se"),ylab = "Cumulative CO2 respiration") + geom_vline(xintercept = 8.5, linetype="dotted", size = 0.5) 

p9 <- df %>% 
  ggpubr::ggline(data =., x = "Week_int", y = "mgC_gdrySoil", color = "Treatment",   palette = cols, add = c("mean_se"),ylab = "Extractable-C (mgC/g dry soil)") + geom_vline(xintercept = 8.5, linetype="dotted", size = 0.5) 

ggpubr::ggarrange(p6, p1, p4, p7, p9,p8,  common.legend = TRUE)
...
p1 <- df %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "CN",  fill = "Treatment", palette = cols,add = c("mean_se"), position = position_dodge(0.9), label = TRUE, lab.nb.digits = 3) +
  stat_compare_means( method = "kruskal") 

p2 <- df %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "C",  fill = "Treatment", palette = cols, add = c("mean_se"), position = position_dodge(0.9), label = TRUE, lab.nb.digits = 3)   +
  stat_compare_means( method = "kruskal") 

p3 <- df %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "N",  fill = "Treatment", palette = cols, add = c("mean_se"), position = position_dodge(0.9), label = TRUE, lab.nb.digits = 3)   +
  stat_compare_means( method = "kruskal") 

p4 <- df %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "HC",  fill = "Treatment", palette = cols, add = c("mean_se"), position = position_dodge(0.9), label = TRUE, lab.nb.digits = 3)   +
  stat_compare_means( method = "kruskal") 

p5 <- df %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "H",  fill = "Treatment", palette = cols, add = c("mean_se"), position = position_dodge(0.9), label = TRUE, lab.nb.digits = 3) +
  stat_compare_means( method = "kruskal") 
...
p1 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "CN", color = "Treatment",  palette = "simpsons",add = c("loess")) 

p2 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "C", color = "Treatment",   palette = "simpsons",add = c("loess")) 

p3 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "N",   color = "Treatment",  palette = "simpsons",add = c("loess")) 

p4 <-df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "H", color = "Treatment",    palette = "simpsons",add = c("loess")) 

p5 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "HC", color = "Treatment",   palette = "simpsons",add = c("loess")) 

p6 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "Dieldrin_trans", color = "Treatment",   palette = "simpsons",add = c("loess")) 
...
p7 <- df %>% 
  ggpubr::ggscatter(data =., x = "Week_int", y = "cum_µgCO2.Cgsoil", color = "Treatment",   palette = "simpsons",add = c("loess")) 

#ggpubr::ggarrange(p1,p2, p3, p4, p5, p6, common.legend = TRUE)

ggpubr::ggarrange(p1,p5, p6, p7, common.legend = TRUE)
...
p1 <- ggcorrplot::ggcorrplot(cor(data_united %>% dplyr::filter(Week != "T0")  %>% dplyr::select(-Week, -Treatment, -id, -SampleNo) ))

p2 <- data_united %>% dplyr::filter(Week != "T0")   %>% 
  ggpubr::ggscatter(y = "loss", x = "H", add = "reg.line", color = "Week") + 
  stat_cor(method = "spearman", aes(color = Week, label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) + 
  stat_cor(method = "spearman", aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),label.y = 22) 
ggarrange(p1, p2)
```

```{r, plotsMBCT52, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=FALSE}
library(ggpubr)
## plots of EOC, Week 52
kr.test <- with(dataMBC %>% filter(Week %in% c("T52", "Reference")) , agricolae::kruskal(mgC_gdrySoil,Treatment,group=TRUE, p.adj="holm"))
levels  <- base::levels(dataMBC$Treatment) %>% as.data.frame() 
groups1 <- kr.test$groups %>% rownames_to_column("Treatment")
groups1 <- groups1[match(levels$., groups1$Treatment),]
label <- as.vector(groups1$groups)
EOCplot <- dataMBC %>% 
  #filter(Week %in% c("T0","T52")) %>% 
  filter(Week %in% c("T52")) %>% 
  ggpubr::ggbarplot(data =., x = "Treatment", y = "mgC_gdrySoil",  fill = "Treatment" ,palette = cols, legend = "none",add = c("mean_se"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
 # ggpubr::stat_compare_means(method = "kruskal", label.y = 0.45) +
#  annotate("text", x = groups1$Treatment, y=c(0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4,0.4), label = label) + 
  ggtitle("EOC at T52")


## Boxplots of MBC, Week 52
#kr.test <- with(dataMBC %>% filter(Week == "T52") , agricolae::kruskal(MBC_µgg_drysoil,Treatment,group=TRUE, p.adj="holm"))
#levels  <- base::levels(dataMBC$Treatment) %>% as.data.frame() 
#groups1 <- kr.test$groups %>% rownames_to_column("Treatment")
#groups1 <- groups1[match(levels$., groups1$Treatment),]
#label <- as.vector(groups1$groups)
MBCplot <- dataMBC %>% 
  #filter(Week %in% c("T0","T52")) %>% 
  filter(Week %in% c("T52")) %>% 
  ggpubr::ggbarplot(data =., x = "Treatment", y = "MBC_µgg_drysoil",  fill = "Treatment", palette = cols, legend = "none",add = c("mean_se"), lab.vjust = -2, xlab = FALSE) + 
  ggpubr::rotate_x_text(angle = 45) + 
  ggpubr::stat_compare_means(method = "kruskal") +
  #annotate("text", x = groups1$Treatment, y=c(150,150,150,150,150,150,150,150,150,150,150), label = label) + 
  ggtitle("MBC at T52")


ggpubr::ggarrange(EOCplot, MBCplot)
```

```{r, plotsBYWEEK, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=FALSE}
library(ggpubr)

### EOC week ###
 #dataMBC %>% 
#  filter(Treatment %in% c("Lime", "H2O2","Inositol", "PoultryLitter", "Biochar", "SodiumFormate", "SodiumAcetate", "FumaricAcid", "Heat", "Control", "CitricAcid")) %>%
#  group_by(Treatment) %>%
#  rstatix::kruskal_test(TOC.g.kg ~ Week) %>%
#  add_significance()
 
p1 <- dataMBC %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "mgC_gdrySoil",  fill = "Week" , palette = "simpsons",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, position = position_dodge(0.9)) + 
  ggpubr::rotate_x_text(angle = 45)  + 
  ggtitle("EOC (mg / g dry soil)") 
  #ggpubr::stat_compare_means(aes(group = Week), label = "p.signif", method = "wilcox.test", paired = FALSE)
...
dataMBC %>% 
  filter(Treatment %in% c("Lime", "H2O2","Inositol", "PoultryLitter", "Biochar", "SodiumFormate", "SodiumAcetate", "FumaricAcid", "Heat","Control", "CitricAcid")) %>%
  group_by(Treatment) %>%
  rstatix::kruskal_test(mgC_gdrySoil ~ Week) %>%
  add_significance()

p2 <- dataMBC %>% 
  ggpubr::ggbarplot(data =., x = "Treatment", y = "mgC_gdrySoil",  fill = "Week" , palette = "simpsons",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, position = position_dodge(0.9)) + 
  ggpubr::rotate_x_text(angle = 45)  + 
  ggtitle("EOC (mg / g dry soil)") 

### MBC ###
p3 <- dataMBC %>% 
  ggpubr::ggbarplot(data =., x = "Week", y = "MBC_pcntTOC",  fill = "Week" , palette = "simpsons",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, position = position_dodge(0.9)) + 
  ggpubr::rotate_x_text(angle = 45)  + 
  ggtitle("MBC % of TOC")

### MBC ratio or pcnt ###
p4 <- dataMBC %>% 
  ggpubr::ggbarplot(data =., x = "Treatment", y = "MBC_pcntTOC",  fill = "Week" , palette = "simpsons",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, position = position_dodge(0.9)) + 
  ggpubr::rotate_x_text(angle = 45)  + 
  ggtitle("MBC % of TOC")  
 # ggpubr::stat_compare_means(aes(group = Week), label = "p.signif", method = "wilcox.test")


ggpubr::ggarrange(p1, p2, p3, p4)
```

```{r lineplotsMBC, message=FALSE, echo = FALSE, warning = FALSE, include=FALSE, eval=FALSE}
### EOC ###
p1 <- dataMBC %>% 
  filter(Treatment != "Reference") %>% 
ggpubr::ggline(data = ., x = "Week_int", y = "mgC_gdrySoil", add = c("mean_se"), color = "Treatment", palette = cols) +
  ggpubr::rotate_x_text(angle = 45) + 
  ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal") + 
  ggtitle("EOC (mg / g dry soil)") 
...
p2 <- dataMBC %>% 
  filter(Treatment != "Reference") %>% 
ggpubr::ggline(data = ., x = "Week_int", y = "MBC_Cratio", add = c("mean_se"), color = "Treatment", palette = cols) +
  ggpubr::rotate_x_text(angle = 45) + 
  ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal") + 
  ggtitle("MBC / TOC ratio") 
...
# Correlations
```{r corplotfig4, fig.cap="Correlations of soil variables.", echo = FALSE, warning = FALSE, cache=TRUE, include=TRUE, fig.width=10, fig.asp=1, include = TRUE }


df <- df_combined %>% dplyr::select(MBC_RespRatio0, MBC_RespRatio39, MBC_RespRatio52, MBC_pcntTOC0, MBC_pcntTOC39 , MBC_pcntTOC52,  EOCT0 , EOC39, EOC52, HC0, HC39, HC52, RespCumT04,RespCumT08,RespCumT16,RespCumT52, TotalLossT39, TotalLossT52)

#p1 <- ggcorrplot::ggcorrplot(cor(df_combined %>% dplyr::select(-Treatment,-RespCumT48, -RespCumT46, -RespCumT44)), type = 'lower')
p1 <- ggcorrplot::ggcorrplot(cor(df, method = "spearman"), type = 'lower')

p2 <- ggcorrplot::ggcorrplot(cor(df_change0_52 %>% dplyr::select(-Treatment), method = "spearman"), type = 'lower')

ggpubr::ggarrange(p1, p2)
#cor.test(df_change0_52$MBC_decrease, df_change0_52$TotalLossT52)
```
...
#Create a df to compare the sum of CO2 produced with other variables
df <- data.frame(Treatment = data_noref[c(1:44),]$Treatment, 
           df1, 
...
## Dieldrin loss
```{r dlossfig5, echo=FALSE, fig.width=10, fig.asp=1.2, include=TRUE, fig.cap="Residues loss in percent. Error bars show the standard error of the mean of four replicates. Letters show differences of the mean after multiple comparison tests (Fisher's least significant difference, holm correct *p* value)." }
...
p1 <- data_noref %>% 
  ...
  filter(SampleName != "averageofthree" ) %>%  # removing T16_31 (Sodium formate), this was measured without internal standard and appeared to have failed
  filter(Week == "T39" ) %>% 
  ggpubr::ggbarplot(data =., x = "Treatment", y = "loss",  fill = "Treatment", palette = cols, facet.by = "Week", legend = "none",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, ylab = "Dieldrin loss (%)") + 
  ggpubr::rotate_x_text(angle = 45) + 
...
p2 <- data_noref %>% 
  ...
  filter(SampleName != "averageofthree" ) %>%  # removing T16_31 (Sodium formate), this was measured without internal standard and appeared to have failed
  filter(Week == "T39" ) %>% 
  ggpubr::ggboxplot(data =., x = "Treatment", y = "loss",  fill = "Treatment", palette = cols, facet.by = "Week", add = c("jitter"), lab.vjust = -2, xlab = FALSE, ylab = "Dieldrin loss (%)") + 
  #ggpubr::rotate_x_text(angle = 45) + 
  ggpubr::stat_compare_means(method = "kruskal", label.y = 12.5,label.x = 2) + 
  geom_hline(yintercept = 0, linetype = 2)  + 
  annotate("text", x = groups1$Treatment, y=c(11,11,11,11,11,11,11,11,11,11,11), label = label) + 
...
p3 <- data_noref %>% 
  ...
  filter(SampleName != "averageofthree" ) %>%  # removing T16_31 (Sodium formate), this was measured without internal standard and appeared to have failed
  filter(Week == "T39" ) %>% 
  ggpubr::ggboxplot(data =., x = "Week", y = "loss",  palette = cols,  facet.by = "Week", add = c("jitter"), lab.vjust = -2, xlab = FALSE, ylab = "Dieldrin loss (%)", add.params = list(shape = 20, color = "Treatment")) + 
  #ggpubr::rotate_x_text(angle = 45) + 
  geom_hline(yintercept = 0, linetype = 2)  + 
  ggtitle("Dieldrin loss from T0 to T39")+
  theme(axis.text.x = element_blank() )
...
```{r lineplotCO2figx, fig.width=8, fig.asp=1,  include = FALSE, message=FALSE, echo = FALSE, cache=FALSE, warning = FALSE, fig.cap="Respiration  (µg CO~2~-C g^-1^ soil) across time. Error bars show the standard error of the mean of four replicates."}
...
p2 <- dataCO2 %>% filter(Week %in% c("T02","T04","T06","T08","T10","T12","T14", "T16")) %>% ggpubr::ggline(data = ., x = "Week_int", y = "µgCO2.Cgsoil", ylab = "Respiration (µg CO2-C/g soil)", add = c("mean_se"), color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p3 <- data_part1  %>% filter(Week %in% c("T02","T04","T06","T08","T10","T12","T14", "T16"))  %>% ggpubr::ggline(data = ., x = "Week_int", y = "µgCO2gsoilcum", ylab = "Respiration accumulated (µg CO2-C/g soil)", add = c("mean_se"), color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")
...
```{r scatterplotx, fig.cap="Scatterplot of total dieldrin loss (%) and total microbial respiration of the first 16 weeks (µg CO~2~-C g^-1^ soil).", include = FALSE, echo = FALSE, warning = FALSE, cache=FALSE, fig.width=8, fig.asp=1}
...
#p1 <- ggscatter(data = df_combined, x= "RespCumT04", y = "TotalLossT52", color = "Treatment", palette = cols,ylab = c("Total dieldrin loss after 39 weeks (%)"), xlab = ("Acum. microbial respiration T04 (µg CO2-C / g soil)")) + stat_cor(method = "spearman",
#   aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

p2 <- ggscatter(data = df_combined, x= "RespCumT08", y = "TotalLossT52", color = "Treatment", palette = cols, ylab = c("Total dieldrin loss after 52 weeks (%)"), xlab = ("Acum. microbial respiration T08 (µg CO2-C / g soil)"))+ stat_cor(method = "spearman",
   aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

p3 <- ggscatter(data = df_combined, x= "RespCumT16", y = "TotalLossT52", color = "Treatment", palette = cols, ylab = c("Total dieldrin loss after 52 weeks (%)"), xlab = ("Acum. microbial respiration T16 (µg CO2-C / g soil)"))+ stat_cor(method = "spearman",
   aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

#p4 <- ggscatter(data = df_combined, x= "RespCumT52", y = "TotalLossT52", color = "Treatment", palette = cols, ylab = c("Total dieldrin loss after 52 weeks (%)"), xlab = ("Acum. microbial respiration T16 (µg CO2-C / g soil)"))+ stat_cor(method = "spearman", aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )


p5 <- df_combined %>% ggpubr::ggscatter(y = "RespCumT52", x = "CN52",color = "Treatment", palette = cols, ylab = "Acum. microbial respiration T52 (µg CO2-C / g soil)", xlab = "Carbon-to-Nitrogen ratio after 52 weeks") + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

p6 <- df_combined %>% ggpubr::ggscatter(y = "RespCumT16", x = "CN39",color = "Treatment", palette = cols, ylab = "Acum. microbial respiration T16 (µg CO2-C / g soil)", xlab = "Carbon-to-Nitrogen ratio after 39 weeks") + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

#p5 <- dfCHN %>% ggpubr::ggscatter(y = "DieldrinT52", x = "H52",color = "Treatment", palette = "simpsons", ylab = "Dieldrin concentration after 52 weeks (µg / g soil)", xlab = "Hydrogen concentrations (%)") + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

ggarrange(p2, p3,  p5, p6, common.legend = TRUE, ncol = 2, nrow = 2)
...
p3 <- data_noref %>% filter(SampleNo != 4 ) %>% filter(Week == "T52" ) %>%  ggpubr::ggbarplot(data =., x = "Treatment", y = "loss39",  fill = "Treatment", palette = cols, facet.by = "Week", legend = "none",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, ylab = "Dieldrin loss (%) T39 - T52") + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(method = "kruskal", label.y = 19, label.x = 2) + geom_hline(yintercept = 0, linetype = 2) + annotate("text", x = groups1$Treatment, y=c(17,17,17,17,17,17,17,17,17,17,17), label = label) + ggtitle("Dieldrin loss from T39 to T52")
ggarrange(p3, ncol = 1)

```

## Lineplots of respiration over time
```{r lineplotCO2figxx, include = FALSE,  fig.width=8, fig.asp=1.5,  message=FALSE, echo = FALSE, cache=FALSE, warning = FALSE, fig.cap="Respiration  (µg CO~2~-C g^-1^ soil) across time. Error bars show the standard error of the mean of four replicates."}
#p1 <- dataCO2  %>% ggpubr::ggline(data = ., x = "Week_int", y = "µgCO2.Cgsoilday", ylab = "µgCO2.Cgsoilday", add = c("mean_se"), color = "Treatment", palette = "simpsons") + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p2 <- dataCO2 %>% filter(Week %in% c("T42", "T44", "T46", "T48", "T52")) %>% ggpubr::ggline(data = ., x = "Week_int", y = "µgCO2.Cgsoilday", ylab = "Respiration (µg CO2-C/g soil)", add = c("mean_se"), color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p3 <- data_part2  %>% filter(Week %in%c("T42", "T44", "T46", "T48", "T52"))  %>% ggpubr::ggline(data = ., x = "Week_int", y = "µgCO2gsoilcum", ylab = "Respiration accumulated (µg CO2-C/g soil)", add = c("mean_se"), color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")

p4 <- data_part2  %>% filter(Week %in%c("T42", "T44", "T46", "T48", "T52"))  %>% ggpubr::ggscatter(data = ., x = "Week_int", y = "µgCO2gsoilcum", ylab = "Respiration accumulated (µg CO2-C/g soil)", add = c("reg.line"), color = "Treatment", palette = cols) + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(aes(group = Treatment), label = "p.signif", method = "kruskal")
...
```{r scatterplotfig7, fig.cap="Scatterplot of total dieldrin loss (%) and total microbial respiration (µg CO~2~-C g^-1^ soil) for weeks 42 to 52.", echo = FALSE, warning = FALSE, cache=FALSE, fig.width=10, fig.asp=1}
#Calculate the sum of CO2 produced per g of soil and create a df for plotting
...
p1 <- ggscatter(data = df, x= "TotalRespiration", y = "loss39to52", color = "Treatment", palette = cols,ylab = c("Dieldrin loss in weeks 39 to 52 (%)"), xlab = ("Total microbial respiration (µg CO2-C / g soil)")) 

p2 <- ggscatter(data = df %>% filter(Treatment %in% c("Control","Lime","Biochar","PoultryLitter", "Heat", "H2O2","FumaricAcid")), x= "TotalRespiration", y = "loss39to52", color = "Treatment", palette = cols,ylab = c("Dieldrin loss in weeks 39 to 52 (%)"), xlab = ("Total microbial respiration (µg CO2-C / g soil)"))  + stat_cor(method = "spearman", aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )
...
```{r scatterplotfigxxx, include = FALSE, fig.cap="Scatterplot of total dieldrin loss (%) and total microbial respiration combined for weeks 1 - 16 and 42 to 52. Only treatments Control, Poultry Litter, Lime, Biochar, Heat and H202 are shown", echo = FALSE, warning = FALSE, cache=FALSE, fig.width=7, fig.asp=0.75}
# Calculate the sum of CO2 produced per g of soil and create a df for plotting
...
## loss figures
```{r dlossfig8, echo=FALSE, fig.width=6, fig.asp=0.6, include=TRUE, fig.cap="Residues loss in percent. Error bars show the standard error of the mean of four replicates. Letters show differences of the mean after multiple comparison tests (Fisher's least significant difference, holm correct *p* value)."}
...
groups1 <- groups1[match(levels$., groups1$Treatment),]
label <- as.vector(groups1$groups)
data_noref %>% filter(SampleNo != 4 ) %>% filter(Week == "T52" ) %>%  ggpubr::ggbarplot(data =., x = "Treatment", y = "loss",  fill = "Treatment", palette = cols, facet.by = "Week", legend = "none",add = c("mean_se"), lab.vjust = -2, xlab = FALSE, ylab = "Dieldrin loss (%) T0 - T52") + ggpubr::rotate_x_text(angle = 45) + ggpubr::stat_compare_means(method = "kruskal", label.y = 20.5, label.x = 2) + geom_hline(yintercept = 0, linetype = 2) + annotate("text", x = groups1$Treatment, y=c(20,20,20,20,20,20,20,20,20,20,20), label = label) + ggtitle("Dieldrin loss from T0 to T52")

```

## Scatterplots
```{r scatterplots52fig9, echo=FALSE, fig.width=10, fig.asp= 0.4, message=FALSE, warning=FALSE, include=TRUE, fig.cap="Scatterplots that show that microbial growth was more efficient where dieldrin degradation tended to be greater, Spearman correlation coefficents are shown"}
#scatterplot of MBC to totallos

## Growth was more efficient where dieldrin degradation tended to be greater
p0 <- ggscatter(data = df_change0_52, x= "MBC_decrease", y = "TotalLossT52", color = "Treatment", palette = cols,  ylab = ("Total dieldrin loss after 52 weeks (%)"), xlab = ("MBC decrease since T0 (%)")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

p1 <- df_combined %>% ggpubr::ggscatter(x = "MBC_pcntTOC52", y= "TotalLossT52",  color = "Treatment" , palette = cols, xlab = ("Microbial biomass-C after 12 months (% TOC)"), ylab = ("")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

p2 <- ggscatter(data = df_combined, x= "RespCumT08", y = "TotalLossT52", color = "Treatment", palette = cols, ylab = c(""), xlab = ("Cum. respiration T08 (µg CO2-C / g soil)"))+ stat_cor(method = "spearman",
   aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

ggarrange(p0, p1, p2, common.legend = TRUE, nrow = 1)
```
...
```{r scatterplots522fig10, echo = FALSE, fig.width=8, fig.asp= 0.5, message=FALSE, warning = FALSE, include=TRUE, fig.cap="Scatterplots of microbial biomass-C vs microbial CO2 respiration, Spearman correlation coefficents are shown"}

# Microbial growth vs respiration
p3 <- df_combined %>% ggpubr::ggscatter(x = "MBC_pcntTOC0", y= "RespCumT52",  color = "Treatment" , palette = cols, xlab = ("Microbial biomass-C T0 (% TOC)"), ylab = ("Cum. respiration T52 (µg CO2-C / g soil)")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))

p4 <- df_combined %>% ggpubr::ggscatter(x = "MBC_pcntTOC52", y= "RespCumT08",  color = "Treatment" , palette = cols, xlab = ("Microbial biomass-C T52 (% TOC)"), ylab = ("Cum. respiration T08 (µg CO2-C / g soil)")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))
...
```{r scatterplots523fig11, fig.width=8, fig.asp= 0.5, message=FALSE, echo = FALSE, warning = FALSE, include=TRUE, fig.cap="Scatterplots of microbial biomass-C vs extractable organic carbon (EOC), Spearman correlation coefficents are shown"}
#scatterplot of MBC to totallos

# Microbes used up available - C
p5 <- ggscatter(data = df_combined, x= "EOCT0", y = "RespCumT52", color = "Treatment", palette = cols,  ylab = ("Cum. respiration T52 (µg CO2-C / g soil)"), xlab = ("Extractable-organic-C T0 (mgC / g dry soil")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

p6 <- ggscatter(data = df_change0_52, x= "Resp_cum52", y = "EOCloss", color = "Treatment", palette = cols,  ylab = ("EOC decrease since T0 (%)"), xlab = ("Cum. respiration T52 (µg CO2-C / g soil)")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

# Microbes using up the available C were less efficient
p7 <- ggscatter(data = df_combined, x= "EOC52", y = "MBC_pcntTOC52", color = "Treatment", palette = cols,  ylab = ("Microbial biomass-C T52 (% TOC)"), xlab = ("Extractable-organic-C T52 (mgC/g dsoil)")) + stat_cor(method = "spearman",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")) )

ggarrange(p7, p6, common.legend = TRUE)
```
...
# Other analyses
## PCA
```{r pcafig12, echo=FALSE, fig.width=8, fig.asp= 0.5, message=FALSE, warning = FALSE, include=TRUE, fig.cap="Principle component analysis (PCA) of combined, scaled and centered soil data"}
library(tidyverse)
library(broom)
library(knitr)
library(ggfortify)

df <- data.frame(df_combined %>% dplyr::select(MBC_pcntTOC0, MBC_pcntTOC52,  EOCT0 , EOC52,  CN52, RespCumT16,RespCumT52, TotalLossT52, DieldrinT0, DieldrinT52),
                 df_change0_52 %>% dplyr::select(MBC_decrease, Hloss) )

df <- df %>% rename("Dieldrin loss" = TotalLossT52, 
                    "MBC T0" = MBC_pcntTOC0,
                    #"MBC T39" = MBC_pcntTOC39,
                    "MBC T52" = MBC_pcntTOC52,
                    "EOC T0" = EOCT0,
                    "EOC T52" = EOC52,
                    "MBC decrease (%)"= MBC_decrease,
                    "H decrease (%)"= Hloss)


#df <- df_change0_52 %>% dplyr::select(-Treatment)

pca_changedata <- prcomp(df, scale = TRUE, center = TRUE)

factoextra::fviz_eig(pca_changedata)
#factoextra::fviz_pca_ind( pca_changedata,
 #            col.ind = "cos2", # Color by the quality of representation
  #           gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
   #          repel = TRUE     # Avoid text overlapping
    #         )

#factoextra::fviz_pca_var(pca_changedata ,
#             col.var = "contrib", # Color by contributions to the PC
#             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
#             repel = TRUE     # Avoid text overlapping
#             )

factoextra::fviz_pca_biplot(pca_changedata, repel = TRUE,
...
#Dieldrin loss
# See dfCHN from chunk corplotfig5

dfCHN$logCN39 <- log(dfCHN$CN39)
dfCHN$logCN52 <- log(dfCHN$CN52)
dfCHN$logH39 <- log(dfCHN$H39)
dfCHN$logH52 <- log(dfCHN$H52)
dfCHN %>% rstatix::shapiro_test(TotalRespiration, TotalLossT52, CN_diff, H_diff, H52, H39, logH52, logH39, CN39, CN52)

M <- gls(TotalLossT52 ~ TotalRespiration + CN_diff + H_diff , data = dfCHN)
M0 <- lme(TotalLossT52 ~TotalRespiration + CN_diff + H_diff , random =~  1 | Treatment, data = dfCHN)

M <- gls(TotalLossT52 ~ TotalRespiration + logCN52 + logH52 , data = dfCHN)
M0 <- lme(TotalLossT52 ~TotalRespiration + logCN52 + logH52 , random =~  1 | Treatment, data = dfCHN)
anova(M, M0)
summary(M0)

M <- gls(TotalLossT39 ~ TotalRespiration + CN_diff + H_diff  , data = dfCHN)
M0 <- lme(TotalLossT39 ~ TotalRespiration + CN_diff + H_diff , random =~  1 | Treatment, data = dfCHN)
anova(M, M0)
summary(M0)

M <- gls(TotalLossT39 ~ TotalRespiration + logCN39 + logH39 , data = dfCHN)
M0 <- lme(TotalLossT39 ~TotalRespiration + logCN39 + logH39 , random =~  1 | Treatment, data = dfCHN)
anova(M, M0)
summary(M0)


Mbays <- arm::bayesglm(TotalLossT52 ~ TotalRespiration + CN52 + H52, family = gaussian, data = dfCHN, prior.mean = 0, prior.scale = Inf, prior.df = Inf)
summary(Mbays)
