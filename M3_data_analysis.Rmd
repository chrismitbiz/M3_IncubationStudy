...
<br/>
```{r corplotfi2, fig.cap="Correlations of total respiration (µg CO~2~-C g^-1^ soil).", echo = FALSE, warning = FALSE, cache=TRUE }
...
```{r scatterplotfi3, fig.cap="Scatterplot of total dieldrin loss (%) and total respiration (µg CO~2~-C g^-1^ soil).", echo = FALSE, warning = FALSE, cache=TRUE, fig.width=9 }
#Calculate the sum of CO2 produced per g of soil
df <- dataCO2  %>%  select(Samples, Week, µgCO2.Cgsoil) %>% tidyr::spread(Week, µgCO2.Cgsoil) %>% column_to_rownames("Samples") %>% rowSums()
...
```{r lineplotCO2, fig.width=8, fig.asp=0.7,  message=FALSE, echo = FALSE, cache=FALSE, warning = FALSE, fig.cap="Respiration  (µg CO~2~-C g^-1^ soil) across time. Error bars show the standard error of the mean of four replicates."}
...
pdf(NULL)
g1 <- ggpubr::ggarrange(p2, nrow = 1, ncol=1, common.legend = TRUE)
x = dev.off()
g1
...
ggarrange(ggarrange(p1, p3,ncol = 1, common.legend = TRUE), p2, ncol = 1, heights = c(0.66, 0.33), labels = "auto")
...
The treatment-comparison of dieldrin concentrations (T0, T16, T39 and T52 combined) showed that biochar and the heat treatment were significantly lower compared to the control and all other treatments.  
...
# LME models
```{r lme, include=FALSE, eval=FALSE, fig.width=10, fig.asp= 1, message=FALSE, echo = TRUE, warning = FALSE}
library(lme4)
library(lmerTest)
library(car)
library(rafalib)
...
<br/> 
```{r, include=FALSE, eval=FALSE,message=FALSE, echo = FALSE, warning = FALSE}
summary(M1)
```
